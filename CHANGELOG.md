...
- **Character Controller** optional Git pin: **`v0.3.0`** ([com.crazycarrot.charactercontroller](https://github.com/Crazy-Carrot-Studios/com.crazycarrot.charactercontroller) tag matches **`package.json`** 0.3.0 — minimal locomotion, authoring menus, demo scene).
- **README:** version banner **0.2.46**; optional controller paragraph updated for **v0.3.0**.

## [0.2.45] - 2026-04-12

### Changed

- **Character Controller** optional Git pin: **`v0.1.0-base-locomotion`** (repository **`main`** rolled back to first public baseline; newer preview work is no longer on **`main`** — see tags **`v0.1.*-preview.1`** for prior commits).
- **README:** version banner **0.2.45**; optional controller description aligned with **0.1.0** layout (**`Scripts/Profiles/`** default profile).

## [0.2.44] - 2026-04-12

### Changed

- **Character Controller** optional Git pin: **`v0.1.11-preview.1`** (**CS1501** camera profile utility vs older bootstrapped runtime).
- **README:** version banner **0.2.44**.

## [0.2.43] - 2026-04-12

### Changed

- **Character Controller** optional Git pin: **`v0.1.10-preview.1`** (locomotion Animator false **Fall** on spawn / new projects).
- **README:** version banner **0.2.43**.

## [0.2.42] - 2026-04-12

### Changed

- **Character Controller** optional Git pin: **`v0.1.9-preview.1`** (camera rig baseline without asset; wizard **CharacterController** fit to mesh + camera target height).
- **README:** version banner **0.2.42**.

## [0.2.41] - 2026-04-12

### Changed

- **Character Controller** optional Git pin: **`v0.1.8-preview.1`** (fixes editor **`CreateBaselineDefaultsInstance`** / **`CCS_CameraProfileAssetUtility`** CS1501 on fresh Hub installs).
- **README:** version banner **0.2.41**.
...
- **CCSDependencyManifest:** **CCS Character Controller** optional entry now states that **com.unity.inputsystem@1.18.0** and **com.unity.cinemachine@3.1.6** are required for the controller foundation and are delivered by the same Hub **required** first-run installs (not optional).
- **README:** Note under **Optional Installs** clarifies the Character Controller package’s dependency on those Unity packages and that they must remain in the project manifest.
- **Documentation~:** Maintainer section **Character Controller (optional CCS package)** documents the Input System / Cinemachine requirement path.
...
- **UPM Git install:** Unity does not allow non-semver versions in `package.json` `dependencies` (Git URLs are rejected for nested deps). Removed **`com.crazycarrot.branding`** from Hub’s dependency list. **`CCS.Branding.Editor`** was removed from **`CCS.Hub.Editor.asmdef`** — Hub already uses **`CCSHubBrandingUi`** (reflection into branding when present). Branding is still installed on first run via **`CCSDependencyManifest`** / **`CCSPackageInstallService`** (Git URL), not as a nested UPM dependency.

## [0.2.33] - 2026-03-28

### Fixed

- **UPM install from Git:** `com.crazycarrot.branding` is not on the Unity Registry; declaring it as `1.1.0` caused *Package cannot be found* when adding Hub via Git URL. The dependency is now a **Git URL** ([com.crazycarrot.branding](https://github.com/Crazy-Carrot-Studios/com.crazycarrot.branding)) so Package Manager can resolve it.
