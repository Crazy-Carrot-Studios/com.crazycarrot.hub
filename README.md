...
**Version 0.2.7** (same value as `package.json` `"version"`).

To **test first-run auto-setup again** in the same project: **CCS → CCS Hub → Reset first-run setup state (this project)**, then **restart Unity** (Hub stores `setupCompleted` in EditorPrefs per project after optional setup finishes).
